...
/// </summary>
public class SS35Form : Form
{
    private readonly TabControl tabs = new();
    private readonly TabPage tab1 = new("거래처별 원장");
    private readonly TabPage tab2 = new("품목별 입출고");
    ...
    private readonly TextBox dspNm2 = new() { ReadOnly = true };
    private readonly Label lblQty1 = new() { AutoSize = true };
    private readonly Label lblQty2 = new() { AutoSize = true };

    private readonly FastDataGridView grid1 = new();
    private readonly FastDataGridView grid2 = new();
    ...
    private void BuildLayout()
    {
        ...
        btnExcel.Click += (_, _) => pStock.Common.ExcelExporter.Export(
            tabs.SelectedTab == tab1 ? grid1 : grid2,
            tabs.SelectedTab == tab1 ? "거래처별원장" : "품목별입출고");
        btnPrint.Click += (_, _) => pStock.Common.GridPrinter.Print(
            tabs.SelectedTab == tab1 ? grid1 : grid2,
            tabs.SelectedTab == tab1 ? "거래처별원장" : "품목별입출고");

        var dateLbl1 = new Label { Text = "기간", Left = 340, Top = 12, AutoSize = true };
        dtpDate1.Left = 380; dtpDate1.Top = 8; dtpDate1.Width = 110; dtpDate1.Format = DateTimePickerFormat.Short;
        var dateLblT = new Label { Text = "~", Left = 495, Top = 12, AutoSize = true };
        dtpDate2.Left = 510; dtpDate2.Top = 8; dtpDate2.Width = 110; dtpDate2.Format = DateTimePickerFormat.Short;
        top.Controls.AddRange(new Control[] { dateLbl1, dtpDate1, dateLblT, dtpDate2 });

        // Tab1
        var lbl1 = new Label { Text = "거래처코드", Left = 10, Top = 12, AutoSize = true };
        edtCvcod.Left = 90; edtCvcod.Top = 8; edtCvcod.Width = 80;
        edtCvcod.KeyDown += EdtCvcod_KeyDown;
        dspName.Left = 180; dspName.Top = 8; dspName.Width = 200;
        var lblAmtCap = new Label { Text = "미수잔액:", Left = 400, Top = 12, AutoSize = true };
        lblAmt.Left = 470; lblAmt.Top = 12;
        var panel1 = new Panel { Dock = DockStyle.Top, Height = 35 };
        panel1.Controls.AddRange(new Control[] { lbl1, edtCvcod, dspName, lblAmtCap, lblAmt });
        ...
        // Tab2
        var lbl2 = new Label { Text = "거래처코드", Left = 10, Top = 12, AutoSize = true };
        edtCd2.Left = 90; edtCd2.Top = 8; edtCd2.Width = 80;
        edtCd2.KeyDown += EdtCd2_KeyDown;
        dspNm2.Left = 180; dspNm2.Top = 8; dspNm2.Width = 200;
        var lblQty1Cap = new Label { Text = "입고수량:", Left = 400, Top = 12, AutoSize = true };
        lblQty1.Left = 470; lblQty1.Top = 12;
        var lblQty2Cap = new Label { Text = "출고수량:", Left = 560, Top = 12, AutoSize = true };
        lblQty2.Left = 630; lblQty2.Top = 12;
        var panel2 = new Panel { Dock = DockStyle.Top, Height = 35 };
        panel2.Controls.AddRange(new Control[] { lbl2, edtCd2, dspNm2, lblQty1Cap, lblQty1, lblQty2Cap, lblQty2 });
        ...
        btnNew.Click += (_, _) =>
        {
            edtCvcod.Clear(); dspName.Clear(); edtCd2.Clear(); dspNm2.Clear();
            grid1.DataSource = null; grid2.DataSource = null;
            lblAmt.Text = "0"; lblQty1.Text = "0"; lblQty2.Text = "0";
        };
        btnSearch.Click += (_, _) => { if (tabs.SelectedTab == tab1) LoadCustLedger(); else LoadItemIO(); };
