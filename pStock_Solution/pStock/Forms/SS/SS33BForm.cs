...
public partial class SS33BForm : Form
{
    ...
    public SS33BForm()
    {
        ...
        this.edtMmdd = new[] { edtMmdd0, edtMmdd1, edtMmdd2, edtMmdd3 };
        this.edtItnbr = new[] { edtItnbr0, edtItnbr1, edtItnbr2, edtItnbr3 };
        this.edtItdsc = new[] { edtItdsc0, edtItdsc1, edtItdsc2, edtItdsc3 };
        this.edtQty = new[] { edtQty0, edtQty1, edtQty2, edtQty3 };
        this.edtCost = new[] { edtCost0, edtCost1, edtCost2, edtCost3 };
        this.edtAmt = new[] { edtAmt0, edtAmt1, edtAmt2, edtAmt3 };
        this.lblHdr = new[] { lblHdr0, lblHdr1, lblHdr2, lblHdr3, lblHdr4, lblHdr5 };
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            var idx = i;
            edtItnbr[idx].KeyDown += (_, e) => this.EdtItnbr_KeyDown(idx, e);
            this._tip.SetToolTip(edtItnbr[idx], "Enter 키를 누르면 품목을 검색합니다.");
            edtQty[idx].ValueChanged += (_, _) => this.RecalcRow(idx);
            edtCost[idx].ValueChanged += (_, _) => this.RecalcRow(idx);
            edtAmt[idx].ValueChanged += (_, _) => this.RecalcTotal();
        }
    ...
    {
        if (this.SaveEntry()) { this.Saved = true; this.ClearForm(); this.edtCvcod.Focus(); }
    }

    private void BtnOne_Click(object? sender, EventArgs e)
    {
        if (this.SaveEntry()) { this.Saved = true; this.Close(); }
    }
